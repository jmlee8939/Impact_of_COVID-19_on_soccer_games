...
PL <- read.csv('After_COVID_PL.csv')
PL <- PL[,3:ncol(PL)]

LIGA <- read.csv('After_COVID_LIGA.csv')
LIGA <- LIGA[,3:ncol(LIGA)]

SE <- read.csv('After_COVID_SE.csv')
SE <- SE[,3:ncol(SE)]

BL <- read.csv('After_COVID_BL.csv')
BL <- BL[,3:ncol(BL)]
...
all_data_b %>% select(home_total_passes, away_total_passes) %>% summary()
all_data %>% select(home_total_passes, away_total_passes) %>% summary()
